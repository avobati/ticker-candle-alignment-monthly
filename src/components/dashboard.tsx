...
import { generateTickerData, type TickerRow, type Alignment, type Market } from "@/data/mock-data";
...
function SignalBadge({ signal }: { signal: string }) {
  ...
  return (
    <span
      className={`inline-flex h-6 w-full items-center justify-center rounded px-2 font-mono text-[11px] ${colorMap[signal] ?? "bg-white/5 text-white/70 ring-1 ring-white/20"}`}
    >
      {signal}
    </span>
  );
...
function AlignmentBadge({ status }: { status: Alignment }) {
  ...
  return (
    <span
      className={`inline-flex h-7 items-center rounded border px-2 text-[11px] ${map[status]}`}
    >
      {label}
    </span>
  );
...
export function Dashboard() {
  const [data] = useState<TickerRow[]>(() => generateTickerData(11054));
  ...
  const filtered = useMemo(() => {
    ...
    if (!isNaN(mc) && mc > 0) {
      rows = rows.filter(
        (r) => r.weekly.candlesAgo <= mc && r.monthly.candlesAgo <= mc
      );
    }
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      alignedBuy: filteredSet.filter((r) => r.status === "Aligned BUY").length,
      watch: filteredSet.filter(
        (r) => r.status === "Weekly BUY, Monthly Neutral"
      ).length,
      conflict: filteredSet.filter((r) => r.status === "Conflict").length,
      avoid: filteredSet.filter((r) => r.status === "Avoid").length,
    };
  }, [filtered]);
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-5 sm:px-6 lg:px-8">
        ...
        <header className="flex flex-col gap-5 border-b border-white/10 pb-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-sm font-medium uppercase tracking-[0.18em] text-cyan-300">
              Weekly and monthly candle alignment
            </p>
        ...
        <section className="rounded border border-white/10 bg-white/[0.03]">
          <div className="grid gap-3 border-b border-white/10 p-4 sm:grid-cols-2 lg:grid-cols-5">
            <button
              className="rounded border px-4 py-3 text-left transition border-cyan-300/40 bg-cyan-300/10"
              type="button"
              onClick={() => setAlignmentFilter("ALL")}
            >
              <p className="text-sm text-slate-400">Tickers</p>
            ...
            <button
              className={`rounded border px-4 py-3 text-left transition ${
                alignmentFilter === "Aligned BUY"
                  ? "border-emerald-300/40 bg-emerald-300/10"
                  : "border-emerald-300/20 bg-emerald-300/[0.04] hover:border-emerald-300/35"
              }`}
              type="button"
              onClick={() =>
                setAlignmentFilter(
                  alignmentFilter === "Aligned BUY" ? "ALL" : "Aligned BUY"
                )
              }
            >
              <p className="text-sm text-emerald-100">Aligned BUY</p>
              <p className="mt-1 font-mono text-2xl text-white">
                {stats.alignedBuy.toLocaleString()}
              </p>
            </button>
            <button
              className={`rounded border px-4 py-3 text-left transition ${
                alignmentFilter === "Weekly BUY, Monthly Neutral"
                  ? "border-cyan-300/40 bg-cyan-300/10"
                  : "border-cyan-300/20 bg-cyan-300/[0.04] hover:border-cyan-300/35"
              }`}
              type="button"
              onClick={() =>
                setAlignmentFilter(
                  alignmentFilter === "Weekly BUY, Monthly Neutral"
                    ? "ALL"
                    : "Weekly BUY, Monthly Neutral"
                )
              }
            >
              <p className="text-sm text-cyan-100">Weekly BUY, Monthly Neutral</p>
              <p className="mt-1 font-mono text-2xl text-white">
                {stats.watch.toLocaleString()}
              </p>
            </button>
            <button
              className={`rounded border px-4 py-3 text-left transition ${
              ...
              onClick={() =>
                setAlignmentFilter(
                  alignmentFilter === "Conflict" ? "ALL" : "Conflict"
                )
              }
            >
              <p className="text-sm text-amber-100">Conflicts</p>
              <p className="mt-1 font-mono text-2xl text-white">
                {stats.conflict.toLocaleString()}
              </p>
            </button>
            <button
              className={`rounded border px-4 py-3 text-left transition ${
                alignmentFilter === "Avoid"
              ...
              onClick={() =>
                setAlignmentFilter(
                  alignmentFilter === "Avoid" ? "ALL" : "Avoid"
                )
              }
            >
              <p className="text-sm text-slate-400">Avoid</p>
              <p className="mt-1 font-mono text-2xl text-white">
                {stats.avoid.toLocaleString()}
              </p>
            </button>
          </div>
          ...
          <div className="grid gap-3 border-b border-white/10 px-4 py-4 lg:grid-cols-[1fr_minmax(24rem,32rem)] lg:items-end">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-400">
                {data.length.toLocaleString()} matching names · rendering{" "}
            ...
            <form
              ...
              onSubmit={(e) => {
                e.preventDefault();
                setTableFilterTicker(searchInput);
              }}
            >
              <div className="flex gap-2">
                ...
                <button
                  className="inline-flex h-10 min-w-24 items-center justify-center rounded border border-cyan-300/30 bg-cyan-300/10 px-3 text-sm font-medium text-cyan-100 transition hover:border-cyan-200 disabled:cursor-not-allowed disabled:opacity-50"
                  type="submit"
                >
                  Check
                </button>
          ...
          <div className="flex flex-col gap-4 border-b border-white/10 px-4 py-4 xl:flex-row xl:items-center xl:justify-between">
            <div className="flex flex-wrap items-center gap-2 text-sm text-slate-400">
              <span>{data.length.toLocaleString()} total names</span>
              <a
                className="inline-flex h-9 items-center gap-2 rounded border border-white/10 bg-white/[0.03] px-3 text-xs font-medium text-slate-200 transition hover:border-cyan-300/40 hover:text-cyan-100"
            ...
            <div className="grid gap-2 md:grid-cols-[minmax(13rem,1fr)_9rem_9rem_9rem_8rem]">
              <label className="relative">
                ...
                <input
                  ...
                  value={tableFilterTicker}
                  onChange={(e) => setTableFilterTicker(e.target.value)}
                />
              </label>
              <select
                className="h-10 rounded border border-white/10 bg-[#101318] px-3 text-sm text-white outline-none transition focus:border-cyan-300/60"
                value={marketFilter}
                onChange={(e) => {
                  setMarketFilter(e.target.value);
                  setTableFilterMarket(e.target.value);
                }}
              >
              ...
              <select
                ...
                value={weeklyFilter}
                onChange={(e) => {
                  setWeeklyFilter(e.target.value as WeeklyFilter);
                  setTableFilterWeekly(e.target.value as WeeklyFilter);
                }}
              >
              ...
              <select
                ...
                value={monthlyFilter}
                onChange={(e) => {
                  setMonthlyFilter(e.target.value as MonthlyFilter);
                  setTableFilterMonthly(e.target.value as MonthlyFilter);
                }}
              >
              ...
              <input
                className="h-10 rounded border border-white/10 bg-black/20 px-3 text-sm text-white outline-none transition placeholder:text-slate-600 focus:border-cyan-300/60"
                inputMode="numeric"
                min="0"
                ...
                value={maxCandles}
                onChange={(e) => {
                  setMaxCandles(e.target.value);
                  setTableFilterMaxCandles(e.target.value);
                }}
              />
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2 border-b border-white/10 px-4 py-3">
            {ALIGNMENT_FILTERS.map((f) => (
              <button
                ...
                onClick={() =>
                  setAlignmentFilter(
                    alignmentFilter === f.key ? "ALL" : f.key
                  )
                }
              >
                {f.label}{" "}
            ...
            <table className="w-full min-w-[1180px] table-fixed border-collapse text-left text-xs">
              ...
              <thead className="sticky top-0 z-10 bg-[#14171c] text-[11px] uppercase tracking-normal text-slate-400 shadow-[0_1px_0_rgba(255,255,255,0.1)]">
                <tr>
                  ...
                  <th className="border-l border-white/10 px-2 py-2 text-center text-cyan-200" colSpan={4}>
                    Weekly
                  </th>
                  <th className="border-l border-white/10 px-2 py-2 text-center text-violet-200" colSpan={4}>
                    Monthly
                  </th>
                  <th className="border-l border-white/10 px-2 py-2 text-right" rowSpan={2}>
                    Chart
                  </th>
                </tr>
                <tr>
              ...
              <tbody>
                {filtered.map((row) => (
                  <tr
                    key={`${row.symbol}-${row.weekly.scannedAt}-${row.monthly.scannedAt}`}
                    className="border-t border-white/10 transition hover:bg-white/[0.025]"
                  >
                    <td
                      className="truncate px-2 py-3 font-mono font-semibold text-cyan-200"
                      title={row.ticker}
                    >
                      {row.ticker}
                    </td>
                    <td
                      className="truncate px-2 py-3 text-slate-200"
                      title={row.symbolName}
                    >
                      {row.symbolName}
                    </td>
                    <td className="truncate px-2 py-3 text-slate-400">
                      {row.market}
                    </td>
                    ...
                    <td className="border-l border-white/10 px-2 py-3">
                      <SignalBadge signal={row.weekly.signal} />
                      <p className="mt-1 truncate font-mono text-[10px] text-slate-600">
                        {formatDate(row.weekly.scannedAt)}
                      </p>
                    </td>
                    <td className="px-2 py-3 text-right font-mono text-slate-200">
                      {row.weekly.candlesAgo}
                    </td>
                    <td className="px-2 py-3 text-right font-mono text-slate-200">
                      {formatPrice(row.weekly.signalPrice)}
                    </td>
                    <td className="px-2 py-3 text-right font-mono text-white">
                      {formatPrice(row.weekly.currentPrice)}
                    </td>
                    <td className="border-l border-white/10 px-2 py-3">
                      <SignalBadge signal={row.monthly.signal} />
                      <p className="mt-1 truncate font-mono text-[10px] text-slate-600">
                        {formatDate(row.monthly.scannedAt)}
                      </p>
                    </td>
                    <td className="px-2 py-3 text-right font-mono text-slate-200">
                      {row.monthly.candlesAgo}
                    </td>
                    <td className="px-2 py-3 text-right font-mono text-slate-200">
                      {formatPrice(row.monthly.signalPrice)}
                    </td>
                    <td className="px-2 py-3 text-right font-mono text-white">
                      {formatPrice(row.monthly.currentPrice)}
                    </td>
                    <td className="border-l border-white/10 px-2 py-3 text-right">
                      <a
                        aria-label={`Open ${row.ticker} on TradingView`}
                        className="inline-flex h-8 w-8 items-center justify-center rounded border border-cyan-300/30 text-cyan-200 transition hover:border-cyan-200 hover:bg-cyan-300/10"
